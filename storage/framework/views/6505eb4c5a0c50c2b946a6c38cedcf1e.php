<?php \ProtoneMedia\Splade\Facades\SEO::title('Posts'); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.backend.page-title','data' => ['heading' => 'Posts','can' => 'create post','url' => ''.e(route('posts.create')).'','title' => 'Create']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['heading' => 'Posts','can' => 'create post','url' => ''.e(route('posts.create')).'','title' => 'Create']); ?>
...
<?php $component = ProtoneMedia\Splade\Components\Table::resolve(['striped' => true,'for' => $posts] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $component->withAttributes([]); ?>
            <?php $__env->slot('spladeTableCelld229bbf31eaeebc7c88897732d8b932d', function ($post, $key) use ($__env) { ?> 
                <div class="block w-24 h-6 rounded-sm" style="background-color: <?php echo e($post->background); ?>"></div>
             <?php }); ?>
            <?php $__env->slot('spladeTableCell67daf92c833c41c95db874e18fcb2786', function ($post, $key) use ($__env) { ?> 
                <div class="w-60 break-words overflow-hidden whitespace-pre-wrap">
                    <?php echo $post->description; ?>

                </div>
             <?php }); ?>
            <?php $__env->slot('spladeTableCell9acb44549b41563697bb490144ec6258', function ($post, $key) use ($__env) { ?> 
                <?php if($post->status == 'unpublished'): ?>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('approve post')): ?>
                        <Link confirm="You are approving this post"
                              confirm-text="Are you sure?"
                              confirm-button="Yes"
                              cancel-button="No"
                              method="POST"
                              href="<?php echo e(route('posts.approve', $post->id)); ?>"
                              class="rounded-md  mr-2 text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-200 font-medium text-sm inline-flex items-center px-3 py-2 text-center">
                        <i class="fa-solid fa-check mr-1"></i>
                        Approve
                        </Link>
                    <?php endif; ?>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('reject post')): ?>
                        <Link confirm="You are rejecting this post"
                              confirm-text="Are you sure?"
                              confirm-button="Yes"
                              cancel-button="No"
                              method="POST"
                              href="<?php echo e(route('posts.reject', $post->id)); ?>"
                              class="rounded-md text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:ring-orange-200 font-medium text-sm inline-flex items-center px-3 py-2 text-center">
                ...
                <?php endif; ?>
             <?php }); ?>
            <?php $__env->slot('spladeTableCellebb67a4271abe715344471b0f16321f6', function ($post, $key) use ($__env) { ?> 
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit post')): ?>
                    <Link href="<?php echo e(route('posts.edit', $post->id)); ?>"
                ...
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete post')): ?>
                    <Link confirm="You are deleting post"
                          confirm-text="Are you sure?"
                          confirm-button="Yes"
                          cancel-button="No"
                          method="DELETE"
                          href="<?php echo e(route('posts.destroy', $post->id)); ?>"
...
<?php echo $__env->make('backend.layouts.default', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/yoshito/Projects/abashenici/resources/views/backend/pages/post/index.blade.php ENDPATH**/ ?>